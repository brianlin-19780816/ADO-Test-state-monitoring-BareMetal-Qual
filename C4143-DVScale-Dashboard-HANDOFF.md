...
| 初版日期 | 2026-07-28 |
| 最後更新 | 2026-08-28 |
| 最新腳本 | `C4142-R-SCM-Dashboard.user.js`（v1.11.4） |
...
| 檔案 / Key | 說明 |
| --- | --- |
| `C4142-R-SCM-Dashboard.user.js` | 最新主交付物與固定安裝入口，Tampermonkey userscript v1.10.0 |
...
2. 把 `C4142-R-SCM-Dashboard.user.js` 拖進 Chrome 視窗（或 Tampermonkey → Utilities → Import from file），按 Install。
...
1. 舊版本檔名必須最後一次手動改裝 `C4142-R-SCM-Dashboard.user.js`；之後 Tampermonkey 可依 GitHub 固定網址與 `@version` 自動更新。
...
- 最新 userscript：`C4142-R-SCM-Dashboard.user.js`（目前 `@version` 1.8.6）
...
`C4142-R-SCM-Dashboard.user.js`

固定 Raw URL：

`https://raw.githubusercontent.com/brianlin-19780816/ADO-Test-state-monitoring-C4143-CRDv4.1-Qual/main/C4142-R-SCM-Dashboard.user.js`
...
1. 所有正式修改都直接更新固定檔名 `C4142-R-SCM-Dashboard.user.js`。
