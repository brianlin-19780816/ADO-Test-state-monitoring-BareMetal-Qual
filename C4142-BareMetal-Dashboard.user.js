// ==UserScript==
// @name         BareMetal Qual Test Status Dashboard
...
// @homepageURL  https://github.com/brianlin-19780816/ADO-Test-state-monitoring-BareMetal-Qual
// @supportURL   https://github.com/brianlin-19780816/ADO-Test-state-monitoring-BareMetal-Qual/issues
// @updateURL    https://raw.githubusercontent.com/brianlin-19780816/ADO-Test-state-monitoring-BareMetal-Qual/main/C4142-BareMetal-Dashboard.user.js
// @downloadURL  https://raw.githubusercontent.com/brianlin-19780816/ADO-Test-state-monitoring-BareMetal-Qual/main/C4142-BareMetal-Dashboard.user.js
...
/* ------------------------------------------------------------------
 How to use
  1) Install Tampermonkey, import this file, then open the dedicated BareMetal Qual entry:
     https://azurecsi.visualstudio.com/_apis/projects?api-version=6.0#baremetalqual
...
(function () {
  ...
  var isDashboardEntry = !!extensionContext || location.hash.toLowerCase().indexOf("baremetalqual") >= 0;
  if (!isDashboardEntry) return;
  var D = {};
  D.CFG = {"org":"https://azurecsi.visualstudio.com","orgName":"azurecsi","project":"Dev","sourceType":"testPlan","planId":3977295,"suiteId":3977296,"queryId":"","queryUrl":"https://azurecsi.visualstudio.com/Dev/_testPlans/charts?planId=3977295&suiteId=3977296","testResultDays":28};
  if (extensionContext) {
    ...
    D.CFG.queryUrl = D.CFG.orgName === 'azurecsi' && D.CFG.project === 'Dev'
      ? 'https://azurecsi.visualstudio.com/Dev/_testPlans/charts?planId=3977295&suiteId=3977296'
      : D.CFG.org + '/' + encodeURIComponent(D.CFG.project) + '/_queries/query/' + D.CFG.queryId + '/';
  }
  D.DEFAULT_QUERIES = [
    { name: 'BareMetal Qual', org: 'https://azurecsi.visualstudio.com', orgName: 'azurecsi', project: 'Dev', sourceType: 'testPlan', planId: 3977295, suiteId: 3977296, queryId: '', queryUrl: 'https://azurecsi.visualstudio.com/Dev/_testPlans/charts?planId=3977295&suiteId=3977296', builtin: true },
  ...
  D.runQuery = async function () {
    ...
    if (D.CFG.sourceType === 'testPlan') {
      ...
      if (!selectedSuite) {
        throw new Error('The configured BareMetal Test Suite ' + D.CFG.suiteId + ' was not found in Test Plan ' + D.CFG.planId + '.');
      }
      var configSuites = [selectedSuite];
      suiteGroups = {};
      ...
      if (!ids.length) throw new Error('The configured BareMetal Test Suite contains no readable Test Cases.');
    } else {
  ...
  D.stacked = function (labels, perRack, states) {
    function shortSuiteLabel(label, index) {
      var value = String(label || ('Test Suite ' + (index + 1))).replace(/\s+/g, ' ').trim();
